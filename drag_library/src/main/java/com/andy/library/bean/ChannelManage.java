package com.andy.library.bean;

import android.database.SQLException;
import android.util.Log;

import com.andy.library.ChannelBean;
import com.andy.library.dao.ChannelDao;
...
public class ChannelManage {
	...
	public static void initData(List<ChannelBean> listAll){
		if(listAll!=null&&listAll.size()>0){
			...
			for (int i=0;i<listAll.size();i++){
				...
				if (bean.isSelect()){
					item.setOrderId(defaultUserChannels.size()+1);
					defaultUserChannels.add(item);
				}else{
					item.setOrderId(defaultOtherChannels.size()+1);
	...
	public List<ChannelItem> getOtherChannel() {
		if(defaultOtherChannels.size()>0)
